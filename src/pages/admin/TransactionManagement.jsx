...
const TransactionManagement = () => {
  ...
  const handleEdit = (transaction) => {
    if (transaction.status !== "pending") {
      setToastMessage("Only 'pending' transactions can be edited");
      setShowToast(true);
      return;
    }
    setFormData({ id: transaction.id, status: transaction.status });
    setShowForm(true);
  };
  ...
  return (
    <div className="min-h-screen w-full bg-gray-900 flex">
      ...
      >
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white">Manage Transaction</h1>
          <p className="text-gray-400">Manage all transactions here.</p>
        </div>
